...
export interface SystemPromptInternals {
  layers: {
    readonly global: {
      readonly runtimeContextSuppressors: { isEmpty(): boolean }
      readonly variables: { entries(): Iterable<[string, (c: AssembleContext) => string | undefined]> }
      readonly toolProviders: { values(): Iterable<(c: AssembleContext) => { schemas: readonly ToolSchema[]; knownNames?: readonly string[] }> }
    }
    ...
    merge(scope: unknown, pick: (layer: unknown) => unknown): { values(): Iterable<unknown> }
  }
