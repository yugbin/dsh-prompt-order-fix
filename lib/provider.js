...
function orderTools(tools, toolOrder, knownNames) {
    ...
    if (toolOrder === undefined)
        return tools.sort((a, b) => compareNames(a.name, b.name));
...
export class DeterministicSystemPrompt extends SystemPrompt {
    ...
    async assemble(context = {}) {
        ...
        const variables = {};
        for (const [name, provider] of internals.layers.global.variables.entries())
        ...
        const sectionByName = internals.layers.merge(scope, (layer) => layer.sections);
        const contextByName = internals.layers.merge(scope, (layer) => layer.contexts);
        const providers = [
